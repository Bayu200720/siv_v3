...
<?php
  ...
  if(isset($_POST['update_sale'])){
    $req_fields = array('title','quantity','buyer','price','total', 'date' );
    validate_fields($req_fields);
        if(empty($errors)){
          $p_id      = $db->escape((int)$product['id']);
          $s_qty     = $db->escape((int)$_POST['quantity']);
          $s_buyer   = $db->escape($_POST['buyer']);
          $s_total   = $db->escape($_POST['total']);
          $date      = $db->escape($_POST['date']);
          $s_date    = date("Y-m-d", strtotime($date));

          $sql  = "UPDATE sales SET";
          $sql .= " product_id= '{$p_id}',qty={$s_qty},buyer='{$_buyer}',price='{$s_total}',date='{$s_date}'";
          ...
          <th> Action</th>
           ...
           <tbody  id="product_info">
              ...
              <form method="post" action="edit_sale.php?id=<?php echo (int)$sale['id']; ?>">
                ...
                <td id="s_byr">
                  <input type="text" class="form-control" name="buyer" value="<?php echo remove_junk($sale['buyer']);?>">
                </td>
                <td id="s_price">
